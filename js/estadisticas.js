...
// Composicion Poblacional: "Personas de 3 años y más", "Personas de 5 años y más", "Personas de 12 años y más", "Personas de 0 a 14 años de edad", "Personas de 15 a 64 años de edad", "Personas de 65 a 130 años de edad"
// Etnicidad: , "Población masculina que se considera afromexicana o afrodescendiente"
// Discapacidad : "Población con limitación", "Población con limitación para caminar, subir o bajar", "Población con limitación para ver, aún usando lentes", "Población con limitación para hablar o comunicarse", "Población con limitación para oír, aún usando aparato auditivo",
...
const menuDataCalor = {
    ...
    "Distribución Territorial": ["Superficie de la Unidad Territorial", "Perfil Altimétrico", "Medir"],

    "Composición Poblacional": ["Población Total", "Población Femenina", "Población Masculina", "Población de 18 años y más"],                
...
// Distribucion Territorial: , "Espacio por habitante (m²/hab)"
// Discapacidad: "Población con limitación", "Población con limitación para caminar, subir o bajar", "Población con limitación para ver, aún usando lentes", "Población con limitación para hablar o comunicarse", "Población con limitación para oír, aún usando aparato auditivo",
                //  "Población con limitación para vestirse, bañarse o comer", "Población con limitación para recordar o concentrarse",
//Datos indicadores
const menuDataIndicadores = {

    "Distribución Territorial": ["Superficie de la Unidad Territorial respecto a la Demarcación"],

    "Composición Poblacional": [ "Porcentaje de Población de la Unidad Territorial respecto a la Demarcacion", "Densidad de Población (HAB/HECT)", "Relación Hombres-Mujeres", "Porcentaje de Mujeres", "Porcentaje de Hombres", "Población de 0 a 17 años" ,"Población de 18 años y más",
                                "Relación de Dependencia", "Índice de Envejecimiento"],
...
/*******************************************************
            CALCULAR CUANTILES (JEANKS)
************************************************************/
/*********************************************************************************
 *   * FUNCIÓN JENKS NATURAL BREAKS (OPTIMAL CLASSIFICATION) *
**********************************************************************************/
function calcularJenks(valores, numClases = 5) {
    ...
    const valoresFiltrados = valores.filter(v => {
        return v != null && !isNaN(v) && isFinite(v) && typeof v === 'number';
    });
    ...
    // Ordenar valores
    const data = [...valoresFiltrados].sort((a, b) => a - b);
    const n = data.length;
    const min = data[0];
    const max = data[n - 1];
    
    // Caso 1: Todos iguales
    if (min === max) {
        return {
            todosIguales: true,
            valorUnico: min,
            breaks: [min, min, min, min, min, min],
            q1: min, q2: min, q3: min, q4: min,
            max: min, min: min
        };
    }
    
    //  CLAVE: Obtener valores únicos y su frecuencia
    const valoresUnicos = [...new Set(data)].sort((a, b) => a - b);
    const numValoresUnicos = valoresUnicos.length;
    
    console.log('═══════════════════════════════════════');
    console.log('Análisis de datos:');
    console.log('Total valores:', n);
    console.log('Valores únicos:', numValoresUnicos);
    console.log('Lista de únicos:', valoresUnicos);
    
    //  CASO ESPECIAL: 3 o menos valores únicos
    // Usar clasificación manual en lugar de Jenks
    if (numValoresUnicos <= 3) {
        console.log(' Solo 3 o menos valores únicos - Usando clasificación manual');
        
        // Mapear valores únicos a clases de forma inteligente
        if (numValoresUnicos === 2) {
            // 2 valores: Bajo y Alto
            const valorBajo = valoresUnicos[0];
            const valorAlto = valoresUnicos[1];
            
            return {
                clasificacionManual: true,
                tipoClasificacion: 'binaria',
                valoresUnicos: numValoresUnicos,
                mapeoClases: {
                    [valorBajo]: 1,  // Muy Bajo
                    [valorAlto]: 5   // Muy Alto
                },
                breaks: [valorBajo, valorBajo, valorBajo, valorBajo, valorAlto, valorAlto],
                q1: valorBajo,
                q2: valorBajo,
                q3: valorBajo,
                q4: valorAlto,
        ...
        } else if (numValoresUnicos === 3) {
            // 3 valores: Bajo, Medio, Alto
            const valorBajo = valoresUnicos[0];
            const valorMedio = valoresUnicos[1];
            const valorAlto = valoresUnicos[2];
            
            return {
                clasificacionManual: true,
                tipoClasificacion: 'ternaria',
                valoresUnicos: numValoresUnicos,
                mapeoClases: {
                    [valorBajo]: 1,   // Muy Bajo
                    [valorMedio]: 3,  // Medio
                ...
                breaks: [valorBajo, valorBajo, valorMedio, valorMedio, valorAlto, valorAlto],
                q1: valorBajo,
                q2: valorMedio,
                q3: valorMedio,
                q4: valorAlto,
    ...
    //  CASO ESPECIAL: 4-6 valores únicos
    // Usar distribución proporcional simple
    if (numValoresUnicos <= 6) {
        console.log(' Entre 4-6 valores únicos - Usando distribución proporcional');
        
        // Distribuir valores únicos en 5 clases de forma proporcional
        const q1 = valoresUnicos[Math.floor(numValoresUnicos * 0.2)];
        const q2 = valoresUnicos[Math.floor(numValoresUnicos * 0.4)];
        const q3 = valoresUnicos[Math.floor(numValoresUnicos * 0.6)];
        const q4 = valoresUnicos[Math.floor(numValoresUnicos * 0.8)];
        
        return {
            clasificacionManual: true,
            tipoClasificacion: 'proporcional',
            valoresUnicos: numValoresUnicos,
            breaks: [min, q1, q2, q3, q4, max],
            q1: q1,
            q2: q2,
            q3: q3,
            q4: q4,
    ...
    //  CASO NORMAL: Más de 6 valores únicos - Usar Jenks Natural Breaks
    console.log(' Suficientes valores únicos - Usando Jenks Natural Breaks');
    
    try {
        // Implementación estándar de Jenks
        const mat1 = Array(n + 1).fill(null).map(() => Array(numClases + 1).fill(0));
        const mat2 = Array(n + 1).fill(null).map(() => Array(numClases + 1).fill(0));
        ...
        console.log('Jenks breaks:', breaks);
        ...
        return {
            ...
            q3: breaks[3],
            q4: breaks[4],
            max: breaks[5] || breaks[4],
            min: breaks[0]
        };
        
    } catch (error) {
        console.error('Error en Jenks, usando quintiles:', error);
        const quintiles = [
            min,
            data[Math.floor(n * 0.2)],
            data[Math.floor(n * 0.4)],
            data[Math.floor(n * 0.6)],
            data[Math.floor(n * 0.8)],
            max
        ];
...
/**
 * Función auxiliar para clasificar un valor según breaks de Jenks
 */
function clasificarValorJenks(valor, jenks) {
    // Caso 1: Sin datos
    if (jenks.sinDatos) {
        return 1;
    }
    ...
    if (jenks.todosIguales) {
        if (jenks.valorUnico === 0) return 1;
        return 3;
    }
    ...
    //  CASO 4: Clasificación manual (2 o 3 valores únicos)
    if (jenks.clasificacionManual && jenks.mapeoClases) {
        // Buscar el valor en el mapeo directo
        if (jenks.mapeoClases[valor] !== undefined) {
            return jenks.mapeoClases[valor];
        }
        // Si no está en el mapeo, es un error - devolver clase media
        console.warn('Valor no encontrado en mapeo:', valor);
        return 3;
    }
    
    //  CASO 5: Clasificación normal con Jenks o proporcional
    if (valor < jenks.q1) return 1;
    if (valor < jenks.q2) return 2;
    if (valor < jenks.q3) return 3;
    if (valor < jenks.q4) return 4;
    return 5;
}
// /*******************************************************
//  *  FUNCIONES GLOBALES *
...
function llenarTablaUT(ut) {
    ...
    //Secciones completas y parciales
    const seccionesCompletasElement = document.getElementById('info-secciones-completas');
    const seccionesParcialesElement = document.getElementById('info-secciones-parciales');

    if (seccionesCompletasElement && seccionesParcialesElement) {

        seccionesCompletasElement.innerHTML = seccionesCompletas
            ? `<span>Secciones completas:</span> ${seccionesCompletas}`
            : '';

        seccionesParcialesElement.innerHTML = seccionesParciales
            ? `<span>Secciones parciales:</span> ${seccionesParciales}`
            : '';
    }

    document.querySelector('.info-ut-secciones').style.display =
    (seccionesCompletas || seccionesParciales) ? 'block' : 'none';
...
async function initMap() {
    ...
    try {
        ...
        const controlPosition = esMobile
            ? google.maps.ControlPosition.TOP_RIGHT
            : google.maps.ControlPosition.TOP_CENTER;

        // Seleccionar zoom dinámico
        const zoomInicial = esMobile ? 15 : 16;
...
async function cargarSecciones() {
    ...
    try {
        // Usar fetchFromApi con el proxy
        const data = await fetchFromApi('filter/secciones_uts', {
            clave: claveUT,
            secc: '' // Parámetro vacío si es necesario, o quítalo si no
        });

        if (data && data.features) {
            mostrarSecciones(data);
        } else {
            console.warn('No se encontraron secciones o el formato no es válido.');
        }
    } catch (error) {
...
function procesarYCargarGeoJSON(datosCompletos) {
    try {
        ...
        // Calcular clasificación usando Jenks Natural Breaks
        ...
        //USAR JENKS EN LUGAR DE MIN/MAX
        const jenksBreaks = calcularJenks(valoresNumericos, 5);

        console.log('Jenks Natural Breaks calculados:', jenksBreaks);

        // Crear leyenda con Jenks
        crearLeyendaColoresJenks(jenksBreaks);
...
function obtenerColorYClasificacionMejorada(valor, jenksBreaks) {
    if (valor == null || isNaN(valor)) {
        return { color: '#cccccc', nivel: 'Sin datos', indice: -1 };
    }
    
    if (!jenksBreaks || !jenksBreaks.breaks) {
        return { color: '#cccccc', nivel: 'Sin clasificar', indice: -1 };
    }
    ...
    // Clasificar usando Jenks Natural Breaks
    const clasificacion = clasificarValorJenks(valor, jenksBreaks);
    return coloresClasificacion[clasificacion - 1]; // -1 porque clasificación va de 1-5
}

function crearLeyendaColoresJenks(jenksBreaks) {
    console.log('Creando leyenda con Jenks Natural Breaks:', jenksBreaks);
    
    if (!jenksBreaks || !jenksBreaks.breaks) {
        console.error('Jenks breaks inválidos');
        return;
    }
    ...
    const rangos = [
        {
            color: '#d4eddad7',
            nivel: 'Muy Bajo',
            rango: `${jenksBreaks.min.toFixed(2)} - ${jenksBreaks.q1.toFixed(2)}`,
            min: jenksBreaks.min,
            max: jenksBreaks.q1
        },
        {
            color: '#a8d5e8',
            nivel: 'Bajo',
            rango: `${(jenksBreaks.q1 + 0.01).toFixed(2)} - ${jenksBreaks.q2.toFixed(2)}`,
            min: jenksBreaks.q1,
            max: jenksBreaks.q2
        },
        {
            color: '#ffeaa7',
            nivel: 'Medio',
            rango: `${(jenksBreaks.q2 + 0.01).toFixed(2)} - ${jenksBreaks.q3.toFixed(2)}`,
            min: jenksBreaks.q2,
            max: jenksBreaks.q3
        },
        {
            color: '#fab1a0',
            nivel: 'Alto',
            rango: `${(jenksBreaks.q3 + 0.01).toFixed(2)} - ${jenksBreaks.q4.toFixed(2)}`,
            min: jenksBreaks.q3,
            max: jenksBreaks.q4
        },
        {
            color: '#e17055',
            nivel: 'Muy Alto',
            rango: `${(jenksBreaks.q4 + 0.01).toFixed(2)} - ${jenksBreaks.max.toFixed(2)}`,
            min: jenksBreaks.q4,
            max: jenksBreaks.max
        }
    ];
    ...
    // Título de la leyenda con indicación de método
    ...
    titulo.style.cssText = `
        ...
        font-style: italic;
    `;
    // titulo.textContent = 'Clasificación: Natural Breaks (Jenks)';
    container.appendChild(titulo);
    
    // Mostrar rangos de mayor a menor
    rangos.reverse().forEach((item) => {
        const div = document.createElement('div');
    ...
    console.log('Leyenda Jenks creada exitosamente');
}
...
function filtrarGraficaPorClasificacion(clasificacionSeleccionada) {
    ...
    // Calcular Jenks para toda la data
    const todosLosValores = datosActualesDemarcacion
        .map(d => Number(d.datos))
        .filter(v => !isNaN(v))
        .sort((a, b) => a - b);
    const jenksBreaks = calcularJenks(todosLosValores, 5);

    const datosClaseSeleccionada = datosActualesDemarcacion.filter(dato => {
        const valor = Number(dato.datos);
        const colorInfo = obtenerColorYClasificacionMejorada(valor, jenksBreaks);
...
function mostrarInfoWindowOptimizado(event, sinPorcentaje = false) {
    ...
    //  FORMATEAR VALOR SEGÚN sinPorcentaje
    let valorFormateado;
    if (valor != null && !isNaN(valor)) {
        if (sinPorcentaje) {
            // Sin decimales para densidad de población
            valorFormateado = Math.round(valor).toLocaleString('en-US');
        } else {
            // Con 2 decimales y PUNTO (en lugar de coma)
...
//             <span style="color: #666;">Valor:</span>
//             <span style="margin-left: 10px; color: #320547; font-weight: bold; font-size: 15px;">
//                 ${valor != null ? valor.toFixed(2) + (sinPorcentaje ? '' : '%') : 'Sin datos'}
//             </span>
//         </div>
//         <div>
//             <span style="color: #666;">Clasificación:</span>
...
//     // Calcular Jenks para clasificación
//     const jenksBreaks = calcularJenks(valores, 5);

//     // Clasificar datos usando Jenks
//     const gruposClasificados = clasificarDatosEnGruposJenks(datosOrdenados, jenksBreaks);
...
//                             return [
//                                 `${titulo}: ${Number(punto.y).toLocaleString('es-ES', {minimumFractionDigits: 2, maximumFractionDigits: 2})}${sinPorcentaje ? '' : '%'}`,
...
function crearGraficaLineasDemarcacion(datos, titulo, sinPorcentaje = false) {
    const container = document.querySelector('.area-grafica') || document.getElementById('graficaContainer');
    const canvas = document.getElementById('graficaLineasDemarcacion');
    ...
    function ajustarTamañoCanvas() {
        const ancho = container.clientWidth;
        const altura = Math.max(300, ancho * 0.5);
        canvas.width = ancho;
        canvas.height = altura;
        if (graficaLineasDemarcacion) {
            graficaLineasDemarcacion.resize();
    ...
    gruposClasificados.forEach((grupo, index) => {
        if (grupo.datos.length > 0) {
            const datosFiltrados = grupo.datos.filter((_, i) => 
                i % Math.max(1, Math.ceil(grupo.datos.length / 15)) === 0
            );
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: function(context) {
                            const punto = context.raw;
                            const valorNumerico = Number(punto.y);
                            
                            //FORMATEAR SEGÚN sinPorcentaje
                            let valorFormateado;
                            if (sinPorcentaje) {
                                // Sin decimales para densidad de población
                                valorFormateado = Math.round(valorNumerico).toLocaleString('en-US');
                            } else {
                                // Con 2 decimales y PUNTO (en lugar de coma)
                                valorFormateado = valorNumerico.toLocaleString('en-US', {
                                    minimumFractionDigits: 2, 
                                    maximumFractionDigits: 2
                                }) + '%';
                            }
    ...
    console.log('Gráfica creada con sinPorcentaje:', sinPorcentaje);
}
...
function clasificarDatosEnGruposJenks(datos, jenksBreaks) {
    const grupos = [
        { datos: [], rango: [jenksBreaks.min, jenksBreaks.q1], nivel: 'Muy Bajo' },
        { datos: [], rango: [jenksBreaks.q1, jenksBreaks.q2], nivel: 'Bajo' },
        { datos: [], rango: [jenksBreaks.q2, jenksBreaks.q3], nivel: 'Medio' },
        { datos: [], rango: [jenksBreaks.q3, jenksBreaks.q4], nivel: 'Alto' },
        { datos: [], rango: [jenksBreaks.q4, jenksBreaks.max], nivel: 'Muy Alto' }
    ];
    ...
    datos.forEach(dato => {
        ...
        // clasificacion va de 1 a 5, grupos tiene índice 0 a 4
        if (clasificacion >= 1 && clasificacion <= 5) {
            grupos[clasificacion - 1].datos.push(dato);
        }
    });
...
async function mapaTematicoPoligonos(campo) {
    try {
        ...
        const response = await fetchFromApi('filter/poligonos_manzanas', { 
            ut_cve_ut: cve_UT,
            [campo]: '' // ← Computed property name
        });
...
/*********************************************************************************
 *   * FUNCIÓN PARA MOSTRAR MENSAJE CUANDO NO HAY POLÍGONOS *
**********************************************************************************/
function mostrarMensajeSinPoligonos() {
    // Remover mensaje anterior si existe
    const mensajeAnterior = document.getElementById('mensaje-sin-poligonos');
    if (mensajeAnterior) {
        mensajeAnterior.remove();
    }

    const mensaje = document.createElement('div');
    mensaje.id = 'mensaje-sin-poligonos';
    ...
    mensaje.innerHTML = `
        <div style="margin-bottom: 15px;">
            <svg width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="#ff6b6b" stroke-width="2">
        ...
        <p style="
            ...
            line-height: 1.6;
        ">
            No se encontró información de polígonos o mapas de calor disponibles para esta Unidad Territorial.
        </p>
        <p style="
            margin: 0 0 20px 0;
            color: #555;
            ...
            font-weight: 500;
        ">
            Puede consultar el apartado <strong style="color: #4a1f7a;">Por Demarcación</strong> 
            para conocer algunos rubros generales de esta Unidad Territorial.
        </p>
        <button onclick="cerrarMensajeSinPoligonos()" style="
    ...
    setTimeout(() => {
        cerrarMensajeSinPoligonos();
    }, 8000);
}

function cerrarMensajeSinPoligonos() {
    const mensaje = document.getElementById('mensaje-sin-poligonos');
    if (mensaje) {
...
function calcularQuintiles(valores) {
    // Mantener el nombre por compatibilidad, pero usar Jenks
    return calcularJenks(valores, 5);
}

function clasificarValor(valor, quintiles) {
    // Ahora quintiles contiene breaks de Jenks
    return clasificarValorJenks(valor, quintiles);
}
...
function crearEstadisticasDescriptivas(valores, campo) {
    ...
    titulo.style.cssText = `
        ...
        padding-bottom: 5px;
    `;
    titulo.textContent = 'Total de la UT';
...
function crearLeyendaTematica(quintiles, campo) {
    console.log('════════════════════════════════════════');
    console.log('Creando leyenda temática');
    ...
    if (leyendaAnterior) {
        ...
        console.log('Leyenda anterior eliminada');
    }

    //  CASO 1: Sin datos válidos
    if (quintiles && quintiles.sinDatos) {
        ...
        `;
        leyenda.innerHTML = `
            <div style="font-weight: bold; margin-bottom: 8px; text-align: center; font-size: 14px; color: #333;">
                Clasificación (Natural Breaks)
            </div>
            <div style="padding: 10px; background: #f8f9fa; border-radius: 4px; text-align: center;">
                <div style="display: flex; align-items: center; justify-content: center; margin-bottom: 6px;">
                    <div style="width: 20px; height: 15px; background: ${obtenerColorPorClasificacion(1).fill}; 
                                border: 1px solid ${obtenerColorPorClasificacion(1).stroke}; margin-right: 8px; border-radius: 2px;"></div>
                    <strong style="color: #666; font-size: 12px;">Sin Datos</strong>
                </div>
                <p style="margin: 0; font-size: 11px; color: #999;">No hay valores válidos</p>
            </div>
        `;
        document.getElementById('map').appendChild(leyenda);
        console.log(' Leyenda "Sin Datos" creada');
        return;
    }
    
    //  CASO 2: Todos los valores son 0
    if (quintiles && quintiles.todosIguales && quintiles.valorUnico === 0) {
        const leyenda = document.createElement('div');
        leyenda.id = 'leyenda-tematica';
        leyenda.style.cssText = `
            position: absolute;
        ...
        leyenda.innerHTML = `
            ...
            <div style="padding: 10px; background: #f8f9fa; border-radius: 4px; text-align: center;">
                ...
                <p style="margin: 0; font-size: 11px; color: #999;">Todos los valores son 0</p>
            </div>
        `;
        document.getElementById('map').appendChild(leyenda);
        console.log(' Leyenda "Todos 0" creada');
        return;
    }
    
    //  CASO 3: Todos los valores son iguales (pero no 0)
    if (quintiles && quintiles.todosIguales) {
        const leyenda = document.createElement('div');
        leyenda.id = 'leyenda-tematica';
        leyenda.style.cssText = `
            position: absolute;
        ...
        leyenda.innerHTML = `
            ...
            <div style="padding: 10px; background: #f8f9fa; border-radius: 4px; text-align: center;">
                <div style="display: flex; align-items: center; justify-content: center; margin-bottom: 6px;">
                    <div style="width: 20px; height: 15px; background: ${obtenerColorPorClasificacion(3).fill}; 
                                border: 1px solid ${obtenerColorPorClasificacion(3).stroke}; margin-right: 8px; border-radius: 2px;"></div>
                    <strong style="font-size: 12px;">Valor único: ${Math.round(quintiles.valorUnico)}</strong>
                </div>
                <p style="margin: 0; font-size: 11px; color: #999;">Todas las unidades iguales</p>
            </div>
        `;
        document.getElementById('map').appendChild(leyenda);
        console.log(' Leyenda "Valor único" creada');
        return;
    }

    //  CASO 4: Clasificación normal o manual
    
    // Validar quintiles
    if (!quintiles || quintiles.min === undefined || quintiles.max === undefined) {
        console.error('Quintiles inválidos:', quintiles);
        return;
    }
    ...
    //  CREAR RANGOS AL ESTILO QGIS (acumulativos, no repetitivos)
    let rangos = [];
    
    if (quintiles.clasificacionManual && quintiles.mapeoClases) {
        //  Para clasificación manual (2-3 valores únicos)
        // Crear rangos acumulativos basados en los valores únicos
        const valoresUnicos = Object.keys(quintiles.mapeoClases)
            .map(v => parseFloat(v))
            .sort((a, b) => a - b);
        
        console.log('Valores únicos para leyenda:', valoresUnicos);
        
        if (valoresUnicos.length === 2) {
            // Ejemplo: [0, 4] → "0 - 0" (Muy Bajo), "4 - 4" (Muy Alto)
            rangos = [
                { 
                    nivel: 5, 
                    rango: `${Math.round(valoresUnicos[1])} - ${Math.round(valoresUnicos[1])}`,
                    nombre: 'Muy Alto' 
                },
                { 
                    nivel: 1, 
                    rango: `${Math.round(valoresUnicos[0])} - ${Math.round(valoresUnicos[0])}`,
                    nombre: 'Muy Bajo' 
                }
            ];
        } else if (valoresUnicos.length === 3) {
            // Ejemplo: [0, 3, 4] → "0 - 0", "0 - 3", "3 - 4"
            rangos = [
                { 
                    nivel: 5, 
                    rango: `${Math.round(valoresUnicos[2])} - ${Math.round(valoresUnicos[2])}`,
                    nombre: 'Muy Alto' 
                },
                { 
                    nivel: 3, 
                    rango: `${Math.round(valoresUnicos[0])} - ${Math.round(valoresUnicos[1])}`,
                    nombre: 'Medio' 
                },
                { 
                    nivel: 1, 
        ...
        //  Para clasificación Jenks normal
        rangos = [
            { 
                nivel: 5, 
                rango: `${Math.round(quintiles.q4)} - ${Math.round(quintiles.max)}`, 
                nombre: 'Muy Alto' 
            },
            { 
                nivel: 4, 
                rango: `${Math.round(quintiles.q3)} - ${Math.round(quintiles.q4)}`, 
                nombre: 'Alto' 
            },
            { 
                nivel: 3, 
                rango: `${Math.round(quintiles.q2)} - ${Math.round(quintiles.q3)}`, 
                nombre: 'Medio' 
            },
            { 
                nivel: 2, 
                rango: `${Math.round(quintiles.q1)} - ${Math.round(quintiles.q2)}`, 
                nombre: 'Bajo' 
            },
            { 
                nivel: 1, 
                rango: `${Math.round(quintiles.min)} - ${Math.round(quintiles.q1)}`, 
                nombre: 'Muy Bajo' 
            }
        ];
    }
    ...
    // Agregar al mapa
    document.getElementById('map').appendChild(leyenda);
    console.log(' Leyenda normal creada exitosamente');
}
...
function limpiarPoligonosTematicos() {
    ...
    // Limpiar mensaje sin polígonos si existe
    const mensajeSinPoligonos = document.getElementById('mensaje-sin-poligonos');
    if (mensajeSinPoligonos) {
        mensajeSinPoligonos.remove();
    }
}
/*********************************************************************************
...
function crearLeyendaMapaCalor(valores, q1, q2, campo) {
    ...
    const titulo = document.createElement('div');
    titulo.id = 'leyenda-calor-titulo';
    titulo.style.cssText = `
        font-weight: bold;
        margin-bottom: 8px;
        text-align: center;
    ...
    rangos.forEach(item => {
        ...
        textoNivel.id = 'leyenda-calor-nivel';
        ...
        textoRango.id = 'leyenda-calor-rango';
        ...
        if(categoria === 'Distribución Territorial'){
            switch (subtema) {
                case 'Superficie de la Unidad Territorial respecto a la Demarcación':
                    ...
                    break;
                case 'Espacio por habitante (m²/hab)':
                    distribucionTerritorial.espacioPorHab();
                    break;
            ...
            switch (subtema) { 
                case 'Porcentaje de Población de la Unidad Territorial respecto a la Demarcacion':
                    ...
                    break;
                case 'Densidad de Población (HAB/HECT)':
                ...
                case 'Porcentaje de Hombres':
                    ...
                    break;
                case 'Población de 0 a 17 años':
                    poblacion.graficaPob0a17();
                    break;
    ...
    //MAPAS DE CALOR
        ...
        if(categoria === 'Distribución Territorial'){
            ...
            switch (subtema) {
                ...
                case 'Medir':
                    distribucionTerritorial.activarHerramientaMedir();
                    break;
            }
...
class DistribucionTerritorial {
    ...
    limpiarTodo() {
        ...
        // 3. Limpiar mediciones
        this.limpiarHerramientaMedir();
        
        // 4. Limpiar todas las leyendas
        this.limpiarTodasLasLeyendas();
        
        // 5. Restaurar estado del mapa
        this.restaurarEstadoMapa();
        
        // 6. Resetear modo actual
        this.modoActual = null;
    }
    ...
    obtenerIconosSVG() {
        return {
            regla: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M3 3h18v18H3z"/>
                <path d="M3 9h18M9 3v18"/>
            </svg>`,
            
            linea: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <line x1="5" y1="19" x2="19" y2="5"/>
                <circle cx="5" cy="19" r="2" fill="currentColor"/>
                <circle cx="19" cy="5" r="2" fill="currentColor"/>
            </svg>`,
            
            ruta: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polyline points="4,20 8,14 12,18 16,12 20,16"/>
            ...
            poligono: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polygon points="12,2 22,12 17,22 7,22 2,12" fill="rgba(255,255,255,0.2)"/>
            ...
            cerrar: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                <line x1="18" y1="6" x2="6" y2="18"/>
                <line x1="6" y1="6" x2="18" y2="18"/>
            </svg>`,
            
            basura: `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            deshacer: `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M3 9l4-4 4 4"/>
                <path d="M7 5v10a4 4 0 004 4h6"/>
            </svg>`,
            
            check: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
     ...
     ********************************/
    activarHerramientaMedir() {
        console.log('Activando herramienta de medición...');
        
        // Verificar geometry library
        if (!google.maps.geometry) {
            alert('La librería de geometría de Google Maps no está cargada. Asegúrate de incluir &libraries=geometry en tu script.');
            return;
        }
        ...
        staticMap.setOptions({ 
            ...
            disableDoubleClickZoom: true // IMPORTANTE: deshabilitar zoom en doble clic
        });
        
        // Deshabilitar interacción con polígonos de UT
        staticMap.data.setStyle({
            fillColor: 'rgba(255, 255, 255, 0)',
            fillOpacity: 0.3,
            strokeColor: 'black',
        ...
        // Mostrar panel
        this.mostrarPanelMedicion();
    }

    mostrarPanelMedicion() {
        const panelAnterior = document.getElementById('panel-medicion');
        if (panelAnterior) panelAnterior.remove();
        
        const iconos = this.obtenerIconosSVG();
        
        const panel = document.createElement('div');
        panel.id = 'panel-medicion';
        panel.style.cssText = `
            position: absolute;
            top: 120px;
            right: 20px;
        ...
        panel.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; border-bottom: 2px solid #2196F3; padding-bottom: 10px;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    ${iconos.regla}
                    <h3 style="margin: 0; color: #2196F3; font-size: 18px;">Herramientas de Medición</h3>
                </div>
                <button id="cerrar-medicion" style="
                    background: #f44336;
                    color: white;
                    border: none;
                    border-radius: 50%;
                    ...
                    transition: all 0.3s;
                    padding: 0;
                " title="Cerrar herramienta">${iconos.cerrar}</button>
            </div>
            
            <div style="margin-bottom: 15px;">
                <button id="medir-linea" class="btn-medicion" data-modo="linea" style="
                    width: 100%;
                    padding: 12px;
                    margin-bottom: 8px;
                ...
                ">
                    ${iconos.linea}
                    <span>Medir Distancia (Línea recta)</span>
                </button>
                
                <button id="medir-ruta" class="btn-medicion" data-modo="ruta" style="
                    width: 100%;
                    padding: 12px;
                    margin-bottom: 8px;
                ...
                ">
                    ${iconos.ruta}
                    <span>Medir Ruta (Multi-punto)</span>
                </button>
                
                <button id="medir-poligono" class="btn-medicion" data-modo="poligono" style="
                    width: 100%;
                    padding: 12px;
                ...
                ">
                    ${iconos.poligono}
                    <span>Medir Área (Polígono)</span>
                </button>
            </div>
            
            <div id="info-medicion" style="
            ...
            ">
                <div id="resultado-medicion" style="font-size: 14px; line-height: 1.8; color: #1565C0;"></div>
                <div style="display: flex; gap: 8px; margin-top: 12px;">
                    <button id="limpiar-medicion" style="
                        flex: 1;
                        padding: 10px;
                        background: #607D8B;
                        color: white;
                    ...
                    <button id="deshacer-punto" style="
                        flex: 1;
                        padding: 10px;
                        background: #FF5722;
                        color: white;
            ...
            <div id="instrucciones-medicion" style="
                margin-top: 15px;
                padding: 12px;
                background: #E3F2FD;
                border-radius: 8px;
            ...
            ">
                <strong>Selecciona una herramienta para comenzar</strong>
            </div>
        `;
        
        document.getElementById('map').appendChild(panel);
        
        // Event listeners con arrow functions para mantener contexto
        document.getElementById('cerrar-medicion').onclick = () => {
            this.limpiarTodo();
        };
        
        document.getElementById('medir-linea').onclick = () => {
            this.seleccionarModoMedicion('linea');
        };
        
        document.getElementById('medir-ruta').onclick = () => {
            this.seleccionarModoMedicion('ruta');
        };
        
        document.getElementById('medir-poligono').onclick = () => {
            this.seleccionarModoMedicion('poligono');
        };
        
        const btnLimpiar = document.getElementById('limpiar-medicion');
        if (btnLimpiar) {
            btnLimpiar.onclick = () => this.limpiarMedicionActual();
        }
        
        const btnDeshacer = document.getElementById('deshacer-punto');
        if (btnDeshacer) {
            btnDeshacer.onclick = () => this.deshacerUltimoPunto();
        }
        
        // Hover effects para botones
        document.querySelectorAll('.btn-medicion').forEach(btn => {
            btn.onmouseover = () => {
        ...
        const btnCerrar = document.getElementById('cerrar-medicion');
        btnCerrar.onmouseover = () => {
            btnCerrar.style.transform = 'scale(1.1)';
            btnCerrar.style.background = '#d32f2f';
        };
    ...
    /********************************
     *    SELECCIONAR MODO CON FEEDBACK VISUAL    *
     ********************************/
    seleccionarModoMedicion(modo) {
        ...
        // Marcar botón activo
        const btnActivo = document.querySelector(`.btn-medicion[data-modo="${modo}"]`);
        if (btnActivo) {
            btnActivo.style.border = '2px solid white';
            btnActivo.style.opacity = '1';
        }
        
        // Iniciar modo correspondiente
        if (modo === 'linea') {
            this.iniciarMedicionLinea();
        } else if (modo === 'ruta') {
            this.iniciarMedicionRuta();
        } else if (modo === 'poligono') {
            this.iniciarMedicionPoligono();
        }
    }

    iniciarMedicionLinea() {
        
        const iconos = this.obtenerIconosSVG();
        
        document.getElementById('instrucciones-medicion').innerHTML = `
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                ${iconos.linea}
                <strong>Modo: Línea Recta</strong>
            </div>
            • Haz <strong>clic</strong> para marcar el punto inicial<br>
            • Haz <strong>clic nuevamente</strong> para el punto final<br>
            • Se calculará la distancia directa
        `;
        
        // Ocultar botón deshacer para línea
        const btnDeshacer = document.getElementById('deshacer-punto');
        if (btnDeshacer) {
            btnDeshacer.style.display = 'none';
        }
        
        this.configurarListenersMedicion();
    }
    ...
    iniciarMedicionRuta() {
        ...
        document.getElementById('instrucciones-medicion').innerHTML = `
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                ${iconos.ruta}
                <strong>Modo: Ruta Multi-punto</strong>
            </div>
            • Haz <strong>clic</strong> para agregar puntos<br>
            • <strong>Doble clic</strong> para finalizar<br>
            • Botón "Deshacer" para quitar último punto
        `;
        
        const btnDeshacer = document.getElementById('deshacer-punto');
        if (btnDeshacer) {
            btnDeshacer.style.display = 'flex';
        }
        
        this.configurarListenersMedicion();
    }

    iniciarMedicionPoligono() {
        ...
        document.getElementById('instrucciones-medicion').innerHTML = `
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                ${iconos.poligono}
                <strong>Modo: Polígono (Área)</strong>
            </div>
            • Haz <strong>clic</strong> para agregar vértices<br>
            • <strong>Doble clic</strong> para cerrar el polígono<br>
    ...
    /********************************
     *    NUEVA FUNCIÓN: LIMPIAR SIN REINICIAR MODO    *
     ********************************/
    limpiarMedicionActualSinReiniciar() {
        console.log('Limpiando medición sin reiniciar modo...');
        
        // Remover TODOS los listeners primero
        if (this.medicion.listener) {
            google.maps.event.removeListener(this.medicion.listener);
            this.medicion.listener = null;
        }
        if (this.medicion.dblClickListener) {
            google.maps.event.removeListener(this.medicion.dblClickListener);
            this.medicion.dblClickListener = null;
        }
        if (this.medicion.mouseMoveListener) {
            google.maps.event.removeListener(this.medicion.mouseMoveListener);
            this.medicion.mouseMoveListener = null;
        }
        
        // Limpiar marcadores
        if (this.medicion.marcadores) {
            this.medicion.marcadores.forEach(marcador => {
                google.maps.event.clearInstanceListeners(marcador);
                marcador.setMap(null);
            });
            this.medicion.marcadores = [];
        }
        
        // Limpiar líneas
        if (this.medicion.lineas) {
            this.medicion.lineas.forEach(linea => {
                google.maps.event.clearInstanceListeners(linea);
                linea.setMap(null);
            });
            this.medicion.lineas = [];
        }
        
        // Limpiar polígono
        if (this.medicion.poligono) {
            google.maps.event.clearInstanceListeners(this.medicion.poligono);
            this.medicion.poligono.setMap(null);
            this.medicion.poligono = null;
        }
        
        // Limpiar línea temporal
        if (this.medicion.lineaTemporal) {
            this.medicion.lineaTemporal.setMap(null);
            this.medicion.lineaTemporal = null;
        }
        
        // Limpiar InfoWindows
        if (this.medicion.infoWindows) {
            this.medicion.infoWindows.forEach(iw => iw.close());
            this.medicion.infoWindows = [];
        }
        
        // Resetear arrays y estado
        this.medicion.puntos = [];
        this.medicion.finalizada = false;
        
        // Ocultar resultados
        const infoDiv = document.getElementById('info-medicion');
        if (infoDiv) {
        ...
        console.log('Limpieza completada sin reiniciar');
    }


    /********************************
     *    CONFIGURAR LISTENERS MEJORADO    *
     ********************************/
    configurarListenersMedicion() {
        console.log(`Configurando listeners para modo: ${this.medicion.modo}`);
        
        // CRÍTICO: Remover listeners anteriores COMPLETAMENTE
        if (this.medicion.listener) {
            google.maps.event.removeListener(this.medicion.listener);
            this.medicion.listener = null;
        }
        ...
        // Pequeña pausa para asegurar que se removieron
        setTimeout(() => {
            // Listener para clics simples
            this.medicion.listener = google.maps.event.addListener(staticMap, 'click', (event) => {
                if (event && event.stop) event.stop();
                if (event && event.latLng) {
                    this.agregarPuntoMedicion(event.latLng);
                }
            });
            
            // Listener para doble clic (solo para ruta y polígono)
            if (this.medicion.modo === 'ruta' || this.medicion.modo === 'poligono') {
                this.medicion.dblClickListener = google.maps.event.addListener(staticMap, 'dblclick', (event) => {
                    if (event && event.stop) event.stop();
                    
                    // Prevenir que se agregue el punto del doble clic
                    setTimeout(() => {
            ...
            // Listener para mousemove (línea temporal)
            this.medicion.mouseMoveListener = google.maps.event.addListener(staticMap, 'mousemove', (event) => {
                if (this.medicion.puntos.length > 0 && !this.medicion.finalizada && event && event.latLng) {
                    this.actualizarLineaTemporal(event.latLng);
                }
            });
            
            console.log(`Listeners configurados correctamente para ${this.medicion.modo}`);
        }, 50);
    }

    agregarPuntoMedicion(latLng) {
        if (this.medicion.finalizada) return;
        
        console.log(`Agregando punto ${this.medicion.puntos.length + 1}:`, latLng.toString());
        
        this.medicion.puntos.push(latLng);
        
        // Crear marcador con SVG personalizado
        const marcador = new google.maps.Marker({
            position: latLng,
            map: staticMap,
            title: `Punto ${this.medicion.puntos.length}`,
            label: {
                text: `${this.medicion.puntos.length}`,
                color: 'white',
                fontSize: '12px',
                fontWeight: 'bold'
            },
            icon: {
                ...
                scale: 10,
                fillColor: this.obtenerColorModo(),
                fillOpacity: 1,
                strokeColor: 'white',
                strokeWeight: 2
            },
        ...
        this.medicion.marcadores.push(marcador);
        
        // Lógica según modo
        if (this.medicion.modo === 'linea') {
            if (this.medicion.puntos.length === 2) {
                this.finalizarMedicionLinea();
            } else if (this.medicion.puntos.length === 1) {
                this.crearLineaTemporal();
            }
        } else if (this.medicion.modo === 'ruta') {
            this.actualizarRuta();
        } else if (this.medicion.modo === 'poligono') {
            this.actualizarPoligono();
        }
    }

    obtenerColorModo() {
        switch(this.medicion.modo) {
            case 'linea': return '#4CAF50';
            case 'ruta': return '#FF9800';
            case 'poligono': return '#9C27B0';
    ...
    crearLineaTemporal() {
        ...
        this.medicion.lineaTemporal = new google.maps.Polyline({
            path: [this.medicion.puntos[0], this.medicion.puntos[0]],
            geodesic: true,
            strokeColor: this.obtenerColorModo(),
            strokeOpacity: 0.5,
            strokeWeight: 2,
            map: staticMap,
            zIndex: 998,
    ...
    actualizarLineaTemporal(latLng) {
        if (this.medicion.lineaTemporal && this.medicion.puntos.length > 0) {
            const ultimoPunto = this.medicion.puntos[this.medicion.puntos.length - 1];
            this.medicion.lineaTemporal.setPath([ultimoPunto, latLng]);
        }
    }

    finalizarMedicionLinea() {
        // Calcular distancia con Google Geometry Library
        const distancia = google.maps.geometry.spherical.computeDistanceBetween(
            this.medicion.puntos[0],
            this.medicion.puntos[1]
        );
        
        // Crear línea definitiva
        const linea = new google.maps.Polyline({
            path: this.medicion.puntos,
            geodesic: true,
            strokeColor: this.obtenerColorModo(),
            strokeOpacity: 1,
            strokeWeight: 4,
        ...
        this.medicion.lineas.push(linea);
        ...
        const puntoMedio = google.maps.geometry.spherical.interpolate(
            this.medicion.puntos[0],
            this.medicion.puntos[1],
            0.5
        );
        ...
        const infoWindow = new google.maps.InfoWindow({
            position: puntoMedio,
            content: `<div style="padding: 8px; font-family: 'Poppins', sans-serif; font-weight: bold; color: ${this.obtenerColorModo()}; text-align: center;">
                ${this.formatearDistancia(distancia)}
            </div>`,
            disableAutoPan: false
        });
        
        infoWindow.open(staticMap);
        this.medicion.infoWindows.push(infoWindow);
        ...
        this.mostrarResultado(`
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                ${iconos.linea}
                <strong style="color: ${this.obtenerColorModo()};">Distancia Medida:</strong>
            </div>
            <div style="text-align: center;">
                <span style="font-size: 24px; font-weight: bold; color: ${this.obtenerColorModo()};">
                    ${this.formatearDistancia(distancia)}
                </span>
            </div>
        `);
        
        this.medicion.finalizada = true;
        staticMap.setOptions({ draggableCursor: 'grab' });
    ...
    actualizarRuta() {
        ...
        this.medicion.lineas.forEach(linea => linea.setMap(null));
        this.medicion.lineas = [];
        
        // Limpiar InfoWindows anteriores (excepto en segmentos)
        this.medicion.infoWindows.forEach(iw => iw.close());
        this.medicion.infoWindows = [];
        ...
        if (this.medicion.puntos.length >= 2) {
            ...
            for (let i = 0; i < this.medicion.puntos.length - 1; i++) {
                const segmento = google.maps.geometry.spherical.computeDistanceBetween(
                    this.medicion.puntos[i],
                    this.medicion.puntos[i + 1]
                );
            ...
            this.mostrarResultado(`
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                    ${iconos.ruta}
                    <strong style="color: ${this.obtenerColorModo()};">Medición de Ruta:</strong>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span><strong>Puntos:</strong></span>
                    <span style="font-weight: bold;">${this.medicion.puntos.length}</span>
                </div>
                <div style="text-align: center; margin-top: 8px;">
                    <div style="font-size: 12px; color: #666;">Distancia Total:</div>
                    <span style="font-size: 20px; font-weight: bold; color: ${this.obtenerColorModo()};">
                        ${this.formatearDistancia(distanciaTotal)}
                    </span>
                </div>
            `);
        }
    ...
    actualizarPoligono() {
        ...
        if (this.medicion.puntos.length >= 3) {
            this.medicion.poligono = new google.maps.Polygon({
                paths: this.medicion.puntos,
                strokeColor: this.obtenerColorModo(),
                strokeOpacity: 1,
                strokeWeight: 3,
                fillColor: this.obtenerColorModo(),
        ...
        const area = google.maps.geometry.spherical.computeArea(this.medicion.puntos);
        
        let perimetro = 0;
        for (let i = 0; i < this.medicion.puntos.length; i++) {
            const siguiente = (i + 1) % this.medicion.puntos.length;
            perimetro += google.maps.geometry.spherical.computeDistanceBetween(
                this.medicion.puntos[i],
                this.medicion.puntos[siguiente]
            );
        }
        ...
        this.mostrarResultado(`
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                ${iconos.poligono}
                <strong style="color: ${this.obtenerColorModo()};">Medición de Polígono:</strong>
            </div>
            <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                <span><strong>Vértices:</strong></span>
                <span style="font-weight: bold;">${this.medicion.puntos.length}</span>
            </div>
            <div style="margin-top: 8px;">
                <div style="font-size: 12px; color: #666;">Área:</div>
                <div style="font-size: 20px; font-weight: bold; color: ${this.obtenerColorModo()}; text-align: center;">
                    ${this.formatearArea(area)}
                </div>
            </div>
            <div style="display: flex; justify-content: space-between; margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(0,0,0,0.1);">
                <span><strong>Perímetro:</strong></span>
                <span style="font-weight: bold;">${this.formatearDistancia(perimetro)}</span>
            </div>
        `);
        }
    }
    ...
    finalizarMedicion() {
        ...
        (this.medicion.modo === 'ruta' && this.medicion.puntos.length >= 2) ||
        (this.medicion.modo === 'poligono' && this.medicion.puntos.length >= 3);
        if (!cumpleRequisitos) {
            console.log('No se cumplen los requisitos mínimos para finalizar');
            return;
        }
        ...
        if (this.medicion.modo === 'poligono') {
            this.actualizarPoligono();
        } else if (this.medicion.modo === 'ruta') {
            this.actualizarRuta();
        }
        ...
        document.getElementById('instrucciones-medicion').innerHTML = `
            ...
            <div style="margin-top: 6px; font-size: 11px;">
                Usa "Limpiar" para realizar una nueva medición
            </div>
        `;
    ...
    deshacerUltimoPunto() {
        if (this.medicion.puntos.length === 0 || this.medicion.finalizada) {
        console.log('No se puede deshacer: no hay puntos o la medición está finalizada');
        return;
        }

        console.log(`Deshaciendo punto ${this.medicion.puntos.length}`);

        // Remover último punto
        this.medicion.puntos.pop();

        // Remover último marcador
        const ultimoMarcador = this.medicion.marcadores.pop();
        if (ultimoMarcador) {
            ultimoMarcador.setMap(null);
        }

        // Actualizar visualización según modo
        if (this.medicion.modo === 'ruta') {
            this.actualizarRuta();
        } else if (this.medicion.modo === 'poligono') {
            this.actualizarPoligono();
        }

        // Si no quedan puntos, ocultar resultado
        if (this.medicion.puntos.length === 0) {
            const infoDiv = document.getElementById('info-medicion');
            if (infoDiv) {
                infoDiv.style.display = 'none';
            }
    ...
    mostrarResultado(html) {
        const infoDiv = document.getElementById('info-medicion');
        const resultadoDiv = document.getElementById('resultado-medicion');

        if (infoDiv && resultadoDiv) {
    ...
    formatearDistancia(metros) {
        if (metros < 1) {
            return `${(metros * 100).toFixed(2)} cm`;
        } else if (metros < 1000) {
    ...
    formatearArea(metrosCuadrados) {
        if (metrosCuadrados < 1) {
            return `${(metrosCuadrados * 10000).toFixed(2)} cm²`;
        } else if (metrosCuadrados < 10000) {
            return `${metrosCuadrados.toFixed(2)} m²`;
        } else if (metrosCuadrados < 1000000) {
            const hectareas = metrosCuadrados / 10000;
            return `${hectareas.toFixed(4)} ha`;
        } else {
            const km2 = metrosCuadrados / 1000000;
            return `${km2.toFixed(6)} km²`;
        }
    }

    limpiarMedicionActual() {
        console.log('Limpiando medición actual desde botón...');
        
        // Guardar modo actual
        const modoActual = this.medicion.modo;
        
        // Limpiar sin reiniciar
        this.limpiarMedicionActualSinReiniciar();
        
        // Reiniciar el mismo modo
        if (modoActual) {
            this.seleccionarModoMedicion(modoActual);
        }
    }

    limpiarHerramientaMedir() {
        if (!this.medicion) return;

        console.log('Limpiando herramienta de medición completa...');
        ...
        if (this.medicion.marcadores) {
            this.medicion.marcadores.forEach(m => m.setMap(null));
        }
        if (this.medicion.lineas) {
            this.medicion.lineas.forEach(l => l.setMap(null));
        }
        if (this.medicion.poligono) {
            this.medicion.poligono.setMap(null);
        }
        ...
        console.log('Herramienta de medición completamente limpiada');

    }

    /********************************
     *    SELECCIONAR MODO CON FEEDBACK VISUAL - CORREGIDO    *
     ********************************/
    seleccionarModoMedicion(modo) {
        console.log(`Cambiando a modo: ${modo}`);
        
        // PRIMERO: Limpiar TODO el estado actual (crítico)
        this.limpiarMedicionActualSinReiniciar();
    ...
    ********************************/    
    ...
    async espacioPorHab(){
        try {
            ...
            const datosCompletos = {
                ...
                // Campo y metadatos
                campo: 'ind_sup3',
                titulo: 'Espacio por habitante (m²/hab)',
                sinPorcentaje: true
            };
    ...
    async cargarCurvasNivel() {
        ...
        try {
            const cve_UT = clave;
            const response = await fetchFromApi('filter/curvas_uts', { 
                cve_ut: cve_UT, 
                elev: '' 
            });
    ...
    async crearInfoSuperficie() {
        ...
        infoDiv.innerHTML = `
            <div class="info-header">
                ...
                <strong class="titulo-superficie">Superficie de la UT</strong>
            </div>
    ...
    crearLeyendaCurvasNivel(elevacionMin, elevacionMax) {
        ...
        const titulo = document.createElement('div');
        titulo.id = 'titulo-leyenda-curvas';
        titulo.style.cssText = `
            font-weight: bold;
            margin-bottom: 8px;
            text-align: center;
    ...
    async graficaPorcentajeDenPob(){
        try {
            ...
            const datosCompletos = {
                ...
                // Campo y metadatos
                campo: 'ind_sup2',
                titulo: 'Densidad de Población (HAB/HECT)',
    ...
    async graficaPob0a17(){
        try {
            ...
            const datosCompletos = {
                ...
                // Campo y metadatos
                campo: 'ind_pob9',
                titulo: 'Población de 0 a 17 años'
            };
    ...
    ********************************/  
    ...
    async mapaPobTotal() {
        try {
            ...
            const uts = await fetchFromApi('filter/centroides_manzanas', { 
                ut_cve_ut: cve_UT,
                pobtot: ''
            });
            let datosUTS = uts.features;
...
class InfoAdicional {
    ...
    constructor() {
        ...
        this.capas = {
            'zonas_valor_ambiental': {
                endpoint: 'filter/valor_ambiental',
            ...
            'linea_conservacion_ecologica': {
                endpoint: 'filter/suelo_conservacion',
            ...
            'zona_patrimonial': {
                endpoint: 'filter/zona_patrimonal',
            ...
            'autoridad_zona_patrimonio': {
                endpoint: 'filter/zona_autoridad',
    ...
    async cargarCapaPatrimonial(nombreCapa = 'zona_patrimonial') {
        try {
            ...
            // 🔹 Hacer petición a la API usando las variables globales
            const response = await fetchFromApi(configCapa.endpoint, configCapa.parametros);

            // 🔹 Verificar si hay datos
            if (response?.features?.length > 0) {
                this.mostrarCapaEnMapa(nombreCapa, response, configCapa);
                console.log(`Capa '${nombreCapa}' cargada con ${response.features.length} elementos`);
            } else {
